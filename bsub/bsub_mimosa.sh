...
for t1 in $(find data/v5/derivatives/qsiprep -name *desc-masked_T1w.nii.gz); do 
    ...
    bsub -J mimosa_bin_$sub -w mimosa_prob_$sub -ti -o logs/mimosa/$sub.log -e logs/mimosa/$sub.log Rscript -e "neurobase::writenii(neurobase::readnii('$out_prob') >= 0.2, '$out_bin')"
done
