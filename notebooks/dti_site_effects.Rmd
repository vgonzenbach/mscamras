...
null_dist %>% 
  select(rowname, F) %>% 
  unnest() %>% 
  ggplot(aes(x=rowname, y=F)) + 
  geom_violin(draw_quantiles = c(0.95)) + 
  coord_flip() + 
  geom_point(aes(x=rowname, y=F), 
...
observed_F <- ratio_stats %>% 
  ...
  select(rowname, F) %>% 
  pivot_wider(names_from = 'rowname', values_from = 'F')
...
## Permutation Testing (No Hopkins)
...
The plot below shows that the permutation test revealed `r perm_df |> filter(pval_fdr < 0.05) |> nrow()` significant site effects across the 36 outcomes. 
...
# Null models
models_0 <- outcomes %>% 
  lapply(function(x) lmer(reformulate("(1|subject)", x), data=df)) %>% 
  setNames(outcomes)
...
```{r}
p_df <- purrr::map2(models, models_0, test_variance_term) %>% 
  setNames(outcomes) %>% 
...
models_0 <- outcomes %>% 
  lapply(function(x) lmer(reformulate("(1|subject)", x), data=filter(df, site != 'Hopkins'))) %>% 
  setNames(outcomes)
