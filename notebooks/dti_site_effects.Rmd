...
To quantify these site effects, $f^2$ u$R^2$ was computed from the model `outcome ~ (1|subject) + site` (i.e. $y_{ijk}$ following the method in Nakagawa & Schielzeth (2013), which is implemented in `performance::r2_nakagawa`. Thus, the (marginal) $R^2$ quantifies the proportion of variance explained by the fixed effect relative to the total variance:

$$ R^2_{(m)} = \frac{\sigma_{site}^2}{\sigma_{site}^2 + \sigma_{subject}^2 + \sigma_{\epsilon}^2}$$
...
For each of the outcome variables, these two models were compared through an asymptotic Likelihood Ratio Test according to Self & Liang (1987). Thus, the final p-values for these tests were calculated using a 50:50 mixture of a $\chi^2_1$ and $\chi^2_2$ distributions. 

To illustrate effect sizes, ICC for both site and subject random intercepts was computed for the full model using the `performance::icc` function. 95% Confidence Intervals for the full models were computed via bootstrapping ($B=5000$). The subject IDs were sampled with replacement. For each iteration, a bootstrap dataset was constructed by selecting all rows from the original dataset that correspond to the given subject ID. As repetition of subject IDs was possible, duplicate rows were also possible. The 2.5 and 97.5 pecentiles of the overall bootstrap distribution served as the limits of the 95% CI. 
...
list(perm_df1, perm_df2, perm_df3, perm_df4) %>% 
  ...
  bind_rows() %>% 
  add_column(excluded_hopkins = factor(rep(c('No', 'Yes', 'No', 'Yes'), each = 36), levels = c('Yes', 'No')), 
             harmonized = factor(rep(c('No', 'No', 'Yes', 'Yes'), each = 36), levels = c('Yes', 'No')),
             .before='outcome') %>% 
  ggplot(aes(x=outcome, y = pval_fdr, color = harmonized, shape = excluded_hopkins)) + 
  geom_point(alpha = 1) + 
  coord_flip() + 
...
add_column(excluded_hopkins = factor(rep(c('No', 'Yes', 'No', 'Yes'), each = 36), levels = c('Yes', 'No')), 
           harmonized = factor(rep(c('No', 'No', 'Yes', 'Yes'), each = 36), levels = c('Yes', 'No')),
           .before='outcome') %>% 
  ggplot(aes(x=outcome, y = f2, color = harmonized, shape = excluded_hopkins)) + 
  geom_point(alpha = 1) + 
  coord_flip() + 
  scale_color_manual(values = c('dodgerblue', 'red')) + 
  scale_shape_manual(values=c(4, 0)) + 
